...
//        }
     ...
     if (Channel) {
        ...
        for (SI::Loop::Iterator it; pmt.streamLoop.getNext(stream, it); ) {
            ...
            switch (stream.getStreamType()) {
              ...
              case 0x80: // STREAMTYPE_USER_PRIVATE
                      if (cSetup::Get().StandardCompliance == STANDARD_ANSISCTE) { // DigiCipher II VIDEO (ANSI/SCTE 57)
                         Vpid = esPid;
                         Ppid = pmt.getPCRPid();
              ...
              case 0x81: // STREAMTYPE_USER_PRIVATE
                      if (cSetup::Get().StandardCompliance == STANDARD_ANSISCTE) { // ATSC A/53 AUDIO (ANSI/SCTE 57)
                         char lang[MAXLANGCODE1] = { 0 };
                         SI::Descriptor *d;
                         for (SI::Loop::Iterator it; (d = stream.streamDescriptors.getNext(it)); ) {
                             switch (d->getDescriptorTag()) {
              ...
              case 0x82: // STREAMTYPE_USER_PRIVATE
                      if (cSetup::Get().StandardCompliance == STANDARD_ANSISCTE) { // STANDARD SUBTITLE (ANSI/SCTE 27)
                         //TODO
                         break;
                         }
                      // fall through
        ...
        if (cSetup::Get().UpdateChannels >= 2) {
