      ...
      if (now.IsValid())
      {
        ...
        for (SI::Loop::Iterator it; tsEIT.eventLoop.getNext(eitEvent, it);)
        {
          ...
          for (SI::Loop::Iterator it2;
              (d = eitEvent.eventDescriptors.getNext(it2));)
          {
            ...
            case SI::TimeShiftedEventDescriptorTag:
              GetText(d, tsEIT.getTableId(), tsEIT.getOriginalNetworkId(),
                  tsEIT.getTransportStreamId(), strTitle, strShortText,
                  strDescription);
              break;
            ...
            case SI::DishRatingDescriptorTag:
              {
                if (d->getLength() == 4)
                {
                  ...
                  thisEvent->SetParentalRating(
                      (newRating << 10) | (rating & 0x3FF));
            ...
            case SI::LinkageDescriptorTag:
              {
                ...
                cChannelID linkID(ld->getOriginalNetworkId(),
            ...
            case SI::ComponentDescriptorTag:
              {
                ...
                if (1 <= stream && stream <= 6 && type != 0)
                {
                  ...
                  components.push_back(
                      CEpgComponent(stream, type,
                          I18nNormalizeLanguageCode(cd->languageCode), buffer));
                }
                break;
              }
            default:
