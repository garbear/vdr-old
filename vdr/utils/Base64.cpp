...
void Base64::Encode(const uint8_t *input, unsigned int length, std::string &output)
{
...
std::string Base64::Encode(const uint8_t *input, unsigned int length)
{
  std::string output;
...
{
  Encode(reinterpret_cast<const uint8_t*>(input.c_str()), input.size(), output);
}
...
void Base64::Decode(const uint8_t *input, unsigned int length, std::string &output)
{
  if (input == NULL || length == 0)
    return;
...
std::string Base64::Decode(const uint8_t *input, unsigned int length)
{
  std::string output;
...
void Base64::Decode(const std::string &input, std::string &output)
{
  ...
  Decode(reinterpret_cast<const uint8_t*>(input.c_str()), length, output);
}
