...
class CConverterType : public CMutex
{
public:
  ...
  iconv_t GetConverter(CLockObject& converterLock);
...
CConverterType::CConverterType(const std::string& sourceCharset, const std::string& targetCharset, unsigned int targetSingleCharMaxLen /*= 1*/) : CMutex(),
...
CConverterType::CConverterType(enum SpecialCharset sourceSpecialCharset, const std::string& targetCharset, unsigned int targetSingleCharMaxLen /*= 1*/) : CMutex(),
...
CConverterType::CConverterType(const std::string& sourceCharset, enum SpecialCharset targetSpecialCharset, unsigned int targetSingleCharMaxLen /*= 1*/) : CMutex(),
  m_sourceSpecialCharset(NotSpecialCharset),
  m_sourceCharset(sourceCharset),
...
CConverterType::CConverterType(enum SpecialCharset sourceSpecialCharset, enum SpecialCharset targetSpecialCharset, unsigned int targetSingleCharMaxLen /*= 1*/) : CMutex(),
  m_sourceSpecialCharset(sourceSpecialCharset),
  m_sourceCharset(),
  m_targetSpecialCharset(targetSpecialCharset),
  m_targetCharset(),
...
CConverterType::CConverterType(const CConverterType& other) : CMutex(),
...
iconv_t CConverterType::GetConverter(CLockObject& converterLock)
{
  // ensure that this unique instance is locked externally
  if (!converterLock.HoldsMutex(*this))
    return NO_ICONV;
...
public:
  ...
  static CMutex m_critSectionFriBiDi;
};
...
CMutex CCharsetConverter::CInnerConverter::m_critSectionFriBiDi;
...
bool CCharsetConverter::CInnerConverter::stdConvert(StdConversionType convertType, const INPUT& strSource, OUTPUT& strDest, bool failOnInvalidChar /*= false*/)
{
  ...
  CLockObject converterLock(convType);
...
bool CCharsetConverter::CInnerConverter::logicalToVisualBiDi(const std::u32string& stringSrc, std::u32string& stringDst, FriBidiCharType base /*= FRIBIDI_TYPE_LTR*/, const bool failOnBadString /*= false*/)
{
  ...
  CLockObject lock(m_critSectionFriBiDi);
  do
  {
...
CCharsetConverter &CCharsetConverter::Get()
{
  static CCharsetConverter instance;
  return instance;
}
...
{
  std::vector<std::string> vecCharsets = CCharsetConverter::Get().getCharsetLabels();
