...
bool cParser::AddPESPacket(uint8_t *data, int size)
{
  ...
  if (m_IsPusi)
  {
    ...
    else if (m_PesHeaderPtr)
    {
      ...
      {
        isyslog("error parsing pes packet error ");
        Reset();
        return false;
      }
      m_PesHeaderPtr = 0;
    ...
    else
    {
      ...
      {
        isyslog("error parsing pes packet error 2");
        Reset();
        return false;
      }
      data += hdr_len;
  ...
  if (m_PesBuffer == NULL)
  {
    ...
    if (m_PesBuffer == NULL)
    {
      esyslog("cParser::AddPESPacket - malloc failed");
      Reset();
      return false;
    }
  }
  ...
  if (m_PesBufferPtr + size >= m_PesBufferSize)
  {
    ...
    {
      esyslog("cParser::AddPESPacket - max buffer size reached, pid: %d", m_pID);
      Reset();
      return false;
    }
    ...
    if (m_PesBuffer == NULL)
    {
      esyslog("cParser::AddPESPacket - realloc failed");
      Reset();
      return false;
    }
  }
  ...
  , m_pID(pid)
{
  ...
  {
    esyslog("Unrecognised type %i inside stream %i", m_streamType, m_pID);
    return;
  }
}
